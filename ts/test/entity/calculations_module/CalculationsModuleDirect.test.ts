...
describe('CalculationsModuleDirect', async () => {
  ...
  test('direct-load-calculations_module', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-calculations_module', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DATA_USA_TEST_CALCULATIONS_MODULE_ENTID']
