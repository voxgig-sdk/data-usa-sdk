...
describe('TesseractSchemaDirect', async () => {
  ...
  test('direct-list-tesseract_schema', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-tesseract_schema', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DATA_USA_TEST_TESSERACT_SCHEMA_ENTID']
