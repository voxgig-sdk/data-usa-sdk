...
describe('TesseractCubeDirect', async () => {
  ...
  test('direct-load-tesseract_cube', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-tesseract_cube', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DATA_USA_TEST_TESSERACT_CUBE_ENTID']
