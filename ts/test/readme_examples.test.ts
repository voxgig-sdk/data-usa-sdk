...
//     (\`new DataUsaSDK(...)\` / \`DataUsaSDK.test(...)\` become
//     \`DataUsaSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"calculations_module":{"test01":{"id":"test01"}},"economic_complexity_module":{"test01":{"id":"test01"}},"health":{"test01":{"id":"test01"}},"member":{"test01":{"id":"test01"}},"module_status":{"test01":{"id":"test01"}},"route_index_get":{"test01":{"id":"test01"}},"tesseract_cube":{"test01":{"id":"test01"}},"tesseract_module":{"test01":{"id":"test01"}},"tesseract_schema":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DataUsaSDK(...)\` and any
// \`DataUsaSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DataUsaSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DataUsaSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
