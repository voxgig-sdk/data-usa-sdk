...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DATAUSA_TEST_LIVE ||
    'TRUE' === process.env.DATAUSA_TEST_OVERRIDE
  ) {
  ...
  m.DATAUSA_TEST_EXPLAIN = process.env.DATAUSA_TEST_EXPLAIN || m.DATAUSA_TEST_EXPLAIN
