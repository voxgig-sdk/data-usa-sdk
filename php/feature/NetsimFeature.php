...
class DataUsaNetsimFeature extends DataUsaBaseFeature
{
    ...
    private function _simulate(DataUsaContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(DataUsaContext $ctx, array $applied): void
    {
        $client = $this->client;
