...
class RouteIndexGetEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DataUsaSDK::test(null, null);
        $ent = $testsdk->RouteIndexGet(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = route_index_get_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "route_index_get." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DATA_USA_TEST_ROUTE_INDEX_GET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $route_index_get_ref01_data = null;
        if (count($route_index_get_ref01_data_raw) > 0) {
...
function route_index_get_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/route_index_get/RouteIndexGetTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["route_index_get01", "route_index_get02", "route_index_get03"] as $k) {
    ...
    $entid_env_raw = getenv("DATA_USA_TEST_ROUTE_INDEX_GET_ENTID");
