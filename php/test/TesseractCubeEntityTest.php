...
class TesseractCubeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = DataUsaSDK::test(null, null);
        $ent = $testsdk->TesseractCube(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = tesseract_cube_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "tesseract_cube." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DATA_USA_TEST_TESSERACT_CUBE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $tesseract_cube_ref01_data = null;
        if (count($tesseract_cube_ref01_data_raw) > 0) {
...
function tesseract_cube_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/tesseract_cube/TesseractCubeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["tesseract_cube01", "tesseract_cube02", "tesseract_cube03"] as $k) {
    ...
    $entid_env_raw = getenv("DATA_USA_TEST_TESSERACT_CUBE_ENTID");
