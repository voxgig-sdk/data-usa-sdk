...
try {
    ...
    print_r($economiccomplexitymodule->data_get());
} catch (\Throwable $err) {
...
$client = DataUsaSDK::test([
    "entity" => ["tesseractcube" => ["test01" => ["id" => "test01"]]],
]);

// Entity ops return the ENTITY (throws on error);
// call data_get() for the mock record.
$tesseractcube = $client->TesseractCube()->load(["id" => "test01"]);
print_r($tesseractcube->data_get());
```
