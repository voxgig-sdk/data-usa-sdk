...
class DataUsaMakeError
{
    public static function call(?DataUsaContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DataUsaContext([], null);
        }
        $op = $ctx->op ?? new DataUsaOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DataUsaResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DataUsaError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DataUsaError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof DataUsaError) {
