...
local ModuleStatusEntity = {}
ModuleStatusEntity.__index = ModuleStatusEntity


function ModuleStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ModuleStatusEntity)
  self._name = "module_status"
  self._client = client
...
function ModuleStatusEntity:get_name()
  return self._name
end


function ModuleStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ModuleStatusEntity.new(self._client, opts)
end
...
function ModuleStatusEntity:mark_deleted()
  self._deleted = true
end


function ModuleStatusEntity:deleted()
  return true == self._deleted
end


function ModuleStatusEntity:data_set(args)
  if args ~= nil then
...
function ModuleStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ModuleStatusEntity:match_set(args)
  if args ~= nil then
...
function ModuleStatusEntity:match_get()
...
function ModuleStatusEntity:stream(action, args, callopts)
...
function ModuleStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
