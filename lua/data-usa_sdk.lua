...
DataUsaSDK._make_feature = _make_feature


function DataUsaSDK.new(options)
  local self = setmetatable({}, DataUsaSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DataUsaSDK:options_map()
...
function DataUsaSDK:get_utility()
  return Utility.copy(self._utility)
end


function DataUsaSDK:get_root_ctx()
  return self._rootctx
end


function DataUsaSDK:prepare(fetchargs)
...
function DataUsaSDK:direct(fetchargs)
...
-- by passing it.
function DataUsaSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DataUsaSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DataUsaSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DataUsaSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DataUsaSDK.new(sdkopts)
  sdk.mode = "test"
