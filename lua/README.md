...
The Lua SDK for the DataUsa API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:CalculationsModule(nil):load({ id = "example_id" }, nil)
...
local result, err = client:DataUsa(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── data-usa_sdk.lua    -- Main SDK module
...
The main module (`data-usa_sdk`) exports the SDK constructor
