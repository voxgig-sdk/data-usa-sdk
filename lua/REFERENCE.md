...
```lua
local result, err = client:CalculationsModule():load({ extension = "extension", param = {} })
```
...
```lua
local result, err = client:TesseractModule():load({ extension = "extension", cube = "cube", drilldown = "drilldown", measure = "measure" })
```
