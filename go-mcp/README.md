...
[MCP](https://modelcontextprotocol.io) server exposing the DataUsa SDK as
two agent tools — `data-usa_list` and `data-usa_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/data-usa-mcp)
make build

# 2. Provide credentials via the environment
export DATA_USA_APIKEY=sk_live_xxx
...
```jsonc
// data-usa_list: first page of records
{ "entity": "member" }
{ "entity": "member", "query": { } }

// data-usa_load: one record by id
{ "entity": "calculations_module", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/data-usa-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user data-usa \
     -- "$PWD"/dist/*/data-usa-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `data-usa_list` and `data-usa_load` tools now appear
   in new sessions. Ask the agent to *"list member using data-usa"*
   and it calls `data-usa_list` with `{"entity":"member"}`.
...
```sh
export DATA_USA_APIKEY=sk_live_xxx            # API key
export DATA_USA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `data-usa_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `data-usa_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DATA_USA_APIKEY` | API key sent with every request. |
| `DATA_USA_BASE` | Optional override of the API base URL. |
