...
type Args struct {
	Entity string         `json:"entity" jsonschema:"calculations_module | economic_complexity_module | health | member | module_status | route_index_get | tesseract_cube | tesseract_module | tesseract_schema"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.DataUsaSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "data-usa_list",
		Description: "List records from DataUsa. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "data-usa_load",
		Description: "Load a single record from DataUsa. " +
...
func runOp(client *sdk.DataUsaSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.DataUsaSDK, name string) (sdk.DataUsaEntity, error) {
	switch strings.ToLower(name) {
	case "calculations_module":
		return client.CalculationsModule(nil), nil
	case "economic_complexity_module":
		return client.EconomicComplexityModule(nil), nil
	case "health":
		return client.Health(nil), nil
	case "member":
		return client.Member(nil), nil
	case "module_status":
		return client.ModuleStatus(nil), nil
	case "route_index_get":
		return client.RouteIndexGet(nil), nil
	case "tesseract_cube":
		return client.TesseractCube(nil), nil
	case "tesseract_module":
		return client.TesseractModule(nil), nil
	case "tesseract_schema":
		return client.TesseractSchema(nil), nil
