...
const calculationsmodule = await client.CalculationsModule().load({ extension: "example", param: {} })
```
...
calculationsmodule = client.CalculationsModule().load({"extension": "example_extension", "param": {}})
...
$calculationsmodule = $client->CalculationsModule()->load(["extension" => "example_extension", "param" => []]);
print_r($calculationsmodule->data_get());
```
...
local calculationsmodule, err = client:CalculationsModule():load({ extension = "example_extension", param = {} })
print(calculationsmodule)
```
