...
class TesseractModuleEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TesseractModuleEntity(self._client, opts)
    ...
    def data_get(self) -> TesseractModule:
    ...
    def match_get(self) -> TesseractModule:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TesseractModule:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TesseractModule().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: TesseractModuleCreateData, ctrl=None) -> TesseractModule:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
