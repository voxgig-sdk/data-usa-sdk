...
class TestTesseractCubeEntity:

    def test_should_create_instance(self):
        testsdk = DataUsaSDK.test(None, None)
        ent = testsdk.TesseractCube(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _tesseract_cube_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "tesseract_cube." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DATAUSA_TEST_TESSERACT_CUBE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        tesseract_cube_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.tesseract_cube")))
        tesseract_cube_ref01_data = None
        if len(tesseract_cube_ref01_data_raw) > 0:
        ...
        tesseract_cube_ref01_data_dt0_loaded, err = tesseract_cube_ref01_ent.load(tesseract_cube_ref01_match_dt0, None)
        assert err is None
        assert tesseract_cube_ref01_data_dt0_loaded is not None



def _tesseract_cube_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/tesseract_cube/TesseractCubeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DATAUSA_TEST_TESSERACT_CUBE_ENTID"))
