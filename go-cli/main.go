// Command DataUsa-cli is a CLI + REPL for the DataUsa SDK,
...
const prompt = "data-usa"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "calculations_module economic_complexity_module health member module_status route_index_get tesseract_cube tesseract_module tesseract_schema"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DATA_USA_APIKEY (api key), DATA_USA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DATA_USA_APIKEY carries the API key and
	// DATA_USA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DATA_USA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DATA_USA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDataUsaSDK(opts)
