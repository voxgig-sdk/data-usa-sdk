...
boru-driven command-line client **and** interactive REPL for the DataUsa
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/data-usa-cli)
make build
...
./data-usa-cli load 1 calculations_module            # {id:1} shorthand
./data-usa-cli load '{id:1}' calculations_module       # explicit match map

# 5. Override the API base URL for a single call
DATA_USA_BASE=https://api.example.com ./data-usa-cli load 1 calculations_module

# 6. No arguments -> interactive REPL
./data-usa-cli
data-usa> load 1 calculations_module
data-usa> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/data-usa-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/data-usa-cli load 1 calculations_module
   ```
...
```sh
./data-usa-cli load 1 calculations_module          # scalar shorthand for {id:1}
./data-usa-cli load '{id:1}' calculations_module     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `data-usa>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `calculations_module`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/data-usa-cli`. |
...
(`./data-usa-cli <expr>`) and interactively (the REPL), and expressions compose the
