...
{
    private static function utility(): DataUsaUtility
    {
        return new DataUsaUtility();
    }
    ...
    private static function ctx(array $over = []): DataUsaContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DataUsaContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DataUsaOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DataUsaError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DataUsaOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DataUsaMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DataUsaOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DataUsaMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DataUsaOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DataUsaMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DataUsaSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DataUsaMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = DataUsaMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DataUsaSpec([]),
            'response' => null,
            'result' => new DataUsaResult([]),
        ]);
        [, $err] = DataUsaMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DataUsaSpec(['step' => 's']),
            'response' => new DataUsaResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DataUsaResult([]),
        ]);
        [, $err] = DataUsaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DataUsaSpec(['step' => 's']),
            'response' => new DataUsaResponse(self::resp(200, ['v' => 1])),
            'result' => new DataUsaResult([]),
        ]);
        [, $err] = DataUsaMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DataUsaSpec(['step' => 's']),
            'response' => new DataUsaResponse(self::resp(200, ['v' => 2])),
            'result' => new DataUsaResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DataUsaMakeResponse::call($ctx);
    ...
    {
        $preset = new DataUsaResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DataUsaSpec([]),
            'response' => new DataUsaResponse([]),
            'result' => new DataUsaResult([]),
        ...
        [$got, $err] = DataUsaMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DataUsaResult([])]);
        [, $err] = DataUsaMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DataUsaSpec([]), 'result' => null]);
        [, $err] = DataUsaMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DataUsaSpec(['step' => 's']),
            'result' => new DataUsaResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DataUsaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DataUsaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DataUsaSpec(['step' => 's']),
            'result' => new DataUsaResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DataUsaOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DataUsaMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DataUsaResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DataUsaSpec([]),
            'result' => new DataUsaResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DataUsaMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DataUsaMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DataUsaError('boom', 'boom');
        $utility->fetcher = function (DataUsaContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DataUsaSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DataUsaMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DataUsaContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DataUsaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DataUsaResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DataUsaContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DataUsaSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DataUsaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DataUsaResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DataUsaSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DataUsaMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DataUsaMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DataUsaMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DataUsaResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DataUsaDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DataUsaResult(['ok' => false])]);
        $this->expectException(DataUsaError::class);
        DataUsaDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DataUsaResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DataUsaMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DataUsaMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DataUsaBaseFeature();
        $b = new DataUsaBaseFeature();
        DataUsaFeatureAdd::call($ctx, $a);
        DataUsaFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DataUsaBaseFeature
    {
        $f = new DataUsaBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DataUsaFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DataUsaContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DataUsaSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DataUsaPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DataUsaResponse(['headers' => null]),
            'result' => new DataUsaResult([]),
        ]);
        DataUsaResultHeaders::call($ctx);
