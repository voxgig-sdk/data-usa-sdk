...
class FtCtrl extends DataUsaControl
{
...
class FtClient
{
    ...
    private DataUsaUtility $utility;

    public function __construct(DataUsaUtility $utility, array $options)
    {
    ...
    public function get_utility(): DataUsaUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DataUsaContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DataUsaContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DataUsaConfig::make_feature($name);
    ...
    private static function buildUrl(DataUsaSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DataUsaError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DataUsaResponse($fetched);
        }
    ...
    private function fail(DataUsaContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DataUsaContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
