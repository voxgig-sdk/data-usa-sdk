...
func TestTesseractSchemaEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.TesseractSchema(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.TesseractSchema(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		tesseractSchemaRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.tesseract_schema")))
...
func tesseract_schemaBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DATA_USA_TEST_TESSERACT_SCHEMA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DATA_USA_TEST_TESSERACT_SCHEMA_ENTID"])
