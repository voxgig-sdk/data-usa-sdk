...
import (
	...
	omni "github.com/voxgig-sdk/data-usa-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DataUsaError); is {
				out := map[string]any{
					"name":    "DataUsaError",
...
func omniCtx(arg any, client *sdk.DataUsaSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DataUsaSDK); is {
			client = live
