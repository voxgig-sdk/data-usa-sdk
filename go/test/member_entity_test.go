...
func TestMemberEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DATAUSA_TEST_MEMBER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func memberBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DATAUSA_TEST_MEMBER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DATAUSA_TEST_MEMBER_ENTID"])
