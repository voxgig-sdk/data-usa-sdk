...
func (sdk *DataUsaSDK) OptionsMap() map[string]any {
...
func (sdk *DataUsaSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *DataUsaSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *DataUsaSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DataUsaSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *DataUsaSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *DataUsaSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("DataUsaSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *DataUsaSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DataUsaSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("DataUsaSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
