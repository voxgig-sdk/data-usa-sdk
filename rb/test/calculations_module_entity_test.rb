...
class CalculationsModuleEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = calculations_module_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "calculations_module." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DATA_USA_TEST_CALCULATIONS_MODULE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    calculations_module_ref01_data = nil
    if calculations_module_ref01_data_raw.length > 0
...
def calculations_module_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "calculations_module", "CalculationsModuleTestData.json")
  ...
  entid_env_raw = ENV["DATA_USA_TEST_CALCULATIONS_MODULE_ENTID"]
