...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = DataUsaOmni.make_runner(TEST_JSON_FILE, DataUsaSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : DataUsaOmni.errify(err)
    end
