...
class TesseractSchemaDirectTest < Minitest::Test
  def test_direct_list_tesseract_schema
    setup = tesseract_schema_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-tesseract_schema", setup[:live] ? "live" : "unit")
    if _should_skip
...
def tesseract_schema_direct_setup(mockres)
