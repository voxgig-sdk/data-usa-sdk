...
module DataUsaUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = DataUsaResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DataUsaResponse.new(fetched)
    else
