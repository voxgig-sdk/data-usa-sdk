...
class EconomicComplexityModuleEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "economic_complexity_module"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    EconomicComplexityModuleEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DataUsaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [EconomicComplexityModule, Hash] the current EconomicComplexityModule data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DataUsaHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of EconomicComplexityModule fields)
  def match_get
  ...
  #
  # @param reqmatch [EconomicComplexityModuleLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.EconomicComplexityModule.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [EconomicComplexityModule, Hash] the loaded EconomicComplexityModule; raises DataUsaError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DataUsaHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
